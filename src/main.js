...
const addContentToContainer = (content, container) => {
...
const renderCards = (count) => {
  ...
  addContentToContainer(fragment, cardsContainer);
};
...
const renderFilters = () => {
  ...
  for (const filterName of Filters.NAMES) {
    let filterCount = getRandomNum(0, Filters.MAX_COUNT);
    allFiltersCount += filterCount;

    fragment += createFilter(filterName, filterCount);
  }

  addContentToContainer(createFilter(`all`, allFiltersCount, true) + fragment, filtersContainer);
};
...
filtersLabels.forEach((filter) => {
  filter.addEventListener(`click`, onFilterLabelClick);
});
...
renderCards(currentFilter.value);
